...
// ---- inject the icon selector at the bottom of the component library ----
// The section between the lofi-icon-selector markers in lofi-library.html is
// generated here so the selector stays in sync with the sprite.
// Note: the library inlines the 19-icon core sprite higher up; where ids
// overlap the core drawing wins (first in DOM), which is fine — same style.
const selectorCells = names.map(n => {
  const t = (tags[n] || []).join(' ');
  return `<button class="lfis-cell" data-name="${n}"${t ? ` data-tags="${esc(t)}"` : ''} title="lf-i-${n}"><svg class="lf-icon"><use href="#lf-i-${n}"/></svg><span>${n}</span></button>`;
}).join('\n');

const selector = `<!-- lofi-icon-selector:start (generated by tools/generate-gallery.mjs — do not edit by hand) -->
  <section class="demo-section">
    <h2>Icon selector — all ${names.length}</h2>
    <p>The complete <a href="https://lucide.dev">Lucide</a> set (v${version}) redrawn in scribble style. Click an icon to copy its <code>&lt;use&gt;</code> snippet, <strong>shift-click</strong> to copy the full <code>&lt;symbol&gt;</code> for inlining. Also available as a <a href="lofi-icons.html">standalone page</a> or the raw sprite <a href="lofi-icons.svg" download>lofi-icons.svg</a>.</p>
    <style>
      .lfis-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(104px, 1fr)); gap: .3rem; margin-top: 1.1rem; }
      .lfis-cell { display: flex; flex-direction: column; align-items: center; gap: .15em;
                   background: none; border: none; padding: .5em .2em .35em; cursor: pointer;
                   font-family: 'Patrick Hand', cursive; font-size: .72rem; color: var(--lf-ink-soft);
                   border-radius: 12px 14px 13px 15px / 14px 12px 15px 13px; }
      .lfis-cell:hover { background: var(--lf-fill); color: var(--lf-ink); }
      .lfis-cell .lf-icon { width: 28px; height: 28px; color: var(--lf-ink); }
      .lfis-cell span { max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .lfis-cell.copied { background: var(--lf-highlight); }
      #lfis-count { min-width: 5.5em; display: inline-block; }
    </style>
    ${sprite}
    <div class="lf-row" style="align-items:center">
      <input class="lf-input" id="lfis-q" type="search" placeholder="Search ${names.length} icons by name or tag..." style="max-width:340px">
      <span class="lf-muted" id="lfis-count">${names.length} shown</span>
    </div>
    <div class="lfis-grid" id="lfis-grid">
${selectorCells}
    </div>
  </section>
  <script>
  (() => {
    const q = document.getElementById('lfis-q');
    const count = document.getElementById('lfis-count');
    const data = [...document.querySelectorAll('.lfis-cell')].map(c => ({ c, hay: (c.dataset.name + ' ' + (c.dataset.tags || '')).toLowerCase() }));
    q.addEventListener('input', () => {
      const terms = q.value.toLowerCase().split(/\\s+/).filter(Boolean);
      let shown = 0;
      for (const { c, hay } of data) {
    ...
    document.getElementById('lfis-grid').addEventListener('click', e => {
      const cell = e.target.closest('.lfis-cell');
      if (!cell) return;
      const id = 'lf-i-' + cell.dataset.name;
      const text = e.shiftKey
        ? document.getElementById(id).outerHTML
  ...
  <!-- lofi-icon-selector:end -->`;

const libPath = REPO + '/lofi-library.html';
const lib = readFileSync(libPath, 'utf8');
const markerRe = /<!-- lofi-icon-selector:start[\s\S]*?<!-- lofi-icon-selector:end -->/;
if (!markerRe.test(lib)) throw new Error('lofi-icon-selector markers not found in lofi-library.html');
writeFileSync(libPath, lib.replace(markerRe, selector));
console.log('updated lofi-library.html icon selector section');
